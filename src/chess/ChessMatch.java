...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (testCheck(currentPlayer)) {
			...
			throw new ChessExeption("You can't pu yourself in check");
		}

		ChessPiece movedPiece= (ChessPiece)board.piece(target);
		
		// #special move poromotion
		//Tem de ser promovido antes do chack
		promoted = null;
	...
	public ChessPiece replacePromotedPiece(String type) {
		if (promoted == null) {
			throw new IllegalStateException("Tyhere is no piece to be promoted");
		}
	...
	private Piece makeMove(Position source, Position target) {
		...
		//#Special move castling kings side rook
	...
	private void validateSourcePosition(Position position) {
		...
		if (!board.piece(position).isThereAnyPossibleMove()) {
			throw new ChessExeption("There is no possible moves for the chosen pieces");
		}
	}

	private void validateTargetPosition(Position source, Position target) {
		if (!board.piece(source).possibleMove(target)) {
			throw new ChessExeption("The chosen piece can't move to target positon");
		}
	}
	...
	private boolean testCheckMate(Color color) {
		...
		List<Piece> list = piecesOnTheBord.stream().filter(x -> ((ChessPiece) x).getColor() == opponent(color))
				.collect(Collectors.toList());
